...
public class MobileInfoUtils {
	...
	public static void jumpStartInterface(final Activity activity) {
		if (isOpenAuto(activity)) {
			//一天提醒一次
			SPUtils.put(activity,"AutoStart"+TimeUtil.getYMDime(),true);
			try {
				MaterialDialog materialdialog = new MaterialDialog.Builder(activity)
								.title("推送管理")
								.titleColorRes(me.shetj.base.R.color.colorPrimary)
								.content("由于安卓系统设置，为获取最新的信息推送，请手动开启自启动权限！")
								.positiveText("立即设置")
								.positiveColorRes(me.shetj.base.R.color.colorPrimary)
								.neutralColorRes(me.shetj.base.R.color.blackHintText)
								.onPositive((dialog, which) -> {
									SPUtils.put(activity,"AutoStart",true);
									toSelfSetting(activity);
									dialog.dismiss();
								})
								.onNeutral((dialog, which) -> {
									dialog.dismiss();
									SPUtils.put(activity,"AutoStart"+AppUtils.getAppVersionCode(),true);
								})
	...
	public static boolean isForceStopAutoStartMethodExist(Context context) {
		synchronized (me.shetj.base.tools.app.MobileInfoUtils.class) {
