...
public class STLReader {
    ...
    public Model parserBinStlInAssets(Context context, String fileName) 
        ...
        return parserBinStl(ArmsUtils.getAssetsFile(fileName));
    }
    //解析二进制的Stl文件
    public Model parserBinStl(InputStream in) throws IOException {
        ...
        int facetCount = OpenGLUtils.byte4ToInt(bytes, 0);
    ...
    private void parseModel(Model model, byte[] facetBytes) {
        ...
        try {
            for (int i = 0; i < facetCount; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    float x = OpenGLUtils.byte4ToFloat(facetBytes, stlOffset);
                    float y = OpenGLUtils.byte4ToFloat(facetBytes, stlOffset + 4);
                    float z = OpenGLUtils.byte4ToFloat(facetBytes, stlOffset + 8);
                    stlOffset += 12;
                ...
                short r = OpenGLUtils.byte2ToShort(facetBytes, stlOffset);
