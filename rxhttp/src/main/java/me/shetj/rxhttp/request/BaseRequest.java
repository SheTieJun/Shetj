...
package me.shetj.rxhttp.request;
...
import me.shetj.rxhttp.RxHttp;
import me.shetj.rxhttp.api.ApiService;
import me.shetj.rxhttp.cache.RxCache;
import me.shetj.rxhttp.cache.converter.IDiskConverter;
import me.shetj.rxhttp.cache.model.CacheMode;
import me.shetj.rxhttp.https.HttpsUtils;
import me.shetj.rxhttp.interceptor.BaseDynamicInterceptor;
import me.shetj.rxhttp.interceptor.CacheInterceptor;
import me.shetj.rxhttp.interceptor.CacheInterceptorOffline;
import me.shetj.rxhttp.interceptor.HeadersInterceptor;
import me.shetj.rxhttp.interceptor.NoCacheInterceptor;
import me.shetj.rxhttp.model.HttpHeaders;
import me.shetj.rxhttp.model.HttpParams;
import me.shetj.rxhttp.utils.HttpLog;
import me.shetj.rxhttp.utils.RxUtil;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = RxHttp.getContext();
        RxHttp config = RxHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) cacheTime = RxHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    public void removeCache(String key) {
        RxHttp.getRxCache().remove(key).compose(RxUtil.<Boolean>io_main())
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = RxHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = RxHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0) RxHttp.getCookieJar().addCookies(cookies);
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            Retrofit.Builder builder = RxHttp.getRetrofitBuilder();
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                Retrofit.Builder newBuilder = RxHttp.getRetrofitBuilder();
                if (!TextUtils.isEmpty(baseUrl)) {
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = RxHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = RxHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(RxHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, RxHttp.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(RxHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(RxHttp.getContext(), cacheControlValue);
            ...
            case CACHEANDREMOTEDISTINCT:
                ...
                if (diskConverter == null) {
                    ...
                    final RxCache.Builder cacheBuilder = RxHttp.getRxCache().newBuilder();
